...
namespace ABXR_ImageDetectionAR
{
    public class trackableImageJsonReader : MonoBehaviour
    {
        public static void FillTrackableMap(
            ...
            toInstantiatePrefabs,
            ImageTrackingSceneSetup parsedTrackableImageRoot
        )
        {
            foreach (TrackedImage
                trackableImage
                in
                ...
                if (toTrackImages[trackableImage.id] == null)
                {
                    ...
                    )
                    {
                        if (toInstantiatePrefabs[renderedPrefab.id] == null)
                        {
                            ...
                            toInstantiatePrefabs
                                .Add(renderedPrefab.id, prefabDetails);
                            prefabList
                                .Add(toInstantiatePrefabs[renderedPrefab.id]);
                        }
                    }
                    var trackableImageDetails =
                        new Dictionary<string, dynamic>()
                        {
                            ...
                            {
                                "referenceImageName",
                                trackableImage.referenceImageName
                            },
